...
| High-churn modules | Derived from git history | For each file changed in the last 100 commits, take its parent directory path. Count how many times each directory appears. Top 10 directories by count = active development areas. Example: if `eras/conway/impl/src/Cardano/Ledger/Conway/Rules/Gov.hs` changed, increment count for `eras/conway/impl/src/Cardano/Ledger/Conway/Rules`. |
...
- Low-test-coverage: source directories with no corresponding `test/` or `testlib/` directory at the same package level
