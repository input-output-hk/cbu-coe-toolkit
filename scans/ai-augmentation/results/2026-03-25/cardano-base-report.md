# AAMM — Sample Report: IntersectMBO/cardano-base
...
| Axis | Score | Detail |
|------|-------|--------|
| **AI Readiness** | **60.66 / 100** | Navigate: 94.80, Understand: 42.25, Verify: 59.00. Penalties: -5. |
...
| Benchmark regression | ✓ | files=7, dirs=9, CI regression=1 |
| Strict evaluation discipline | ✗ | StrictData/BangPatterns=0 |
| .aiignore on critical paths | ✗ | 0 |

---

## AI Readiness: 60.66 / 100

### Pillar 1: Navigate — 94.80 / 100 (weight: 0.35)

**Poate AI-ul lucra eficient aici?**

| # | Signal | Score | Weight | Evidence |
|---|--------|-------|--------|----------|
| N1 | file_organization | **100** | 0.12 | max_depth=6, directories=136 |
| N2 | file_granularity | **100** | 0.13 | median_lines=47, large_files=0, per_file_penalty=-0 |
| N3 | module_boundaries | **100** | 0.15 | workspace_files=2, package_manifests=13 |
...
| N6 | cicd_pipeline | **100** | 0.15 | workflows=2, has_build=1, has_deploy=1, days_since_push=1 |
...
### Pillar 2: Understand — 42.25 / 100 (weight: 0.35)
...
| U3 | readme_substance | **40** | 0.15 | readme_lines=75, desc=1, setup=1, usage=0, arch=0, contrib=0 |
...
**Hard Gate:** PASS (39 test files)

| # | Signal | Score | Weight | Evidence |
|---|--------|-------|--------|----------|
| V1 | test_source_ratio | **50** | 0.30 | ratio=.367 (39 test / 106 source) |
...
Readiness_raw = Navigate × 0.35 + Understand × 0.35 + Verify × 0.30
              = 94.80 × 0.35 + 42.25 × 0.35 + 59.00 × 0.30
              = 65.66
...
| Private | false |
| Primary language | Haskell (66.6%) |
| Languages | Haskell: 66.6%,C: 29.9%,Shell: 1.7%,Rust: 0.9%,Nix: 0.8% |
| Size | 12547 KB |
| Open issues | 45 |
| Stars | 104 |
| License | Apache-2.0 |
| Created | 2019-03-18T13:27:33Z |
| Last push | 2026-03-24T10:33:05Z |
| Tree entries | 427 |
| Source files | 106 |
| Test files | 39 |
| Directories | 136 |
...
```
Navigate   = 94.80  (weight 0.35)
Understand = 42.25  (weight 0.35)
...
| nav_und_gap | warning | Navigate (94.80) >> Understand (42.25). Well-structured codebase but AI can't understand intent. Typically means docs/comments gap. High-impact improvement area. | operator_attention |
...
### Improve AI Understanding (Understand: 42.25/100)
