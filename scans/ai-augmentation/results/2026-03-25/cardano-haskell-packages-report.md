# AAMM — Sample Report: IntersectMBO/cardano-haskell-packages
...
| Axis | Score | Detail |
|------|-------|--------|
| **AI Readiness** | **37.46 / 100** | Navigate: 78.25, Understand: 30.25, Verify: 15.00. Penalties: -5. |
...
**Detected via:** repo topics, description
...
| Benchmark regression | ✓ | files=6, dirs=7, CI regression=0 |
...
## AI Readiness: 37.46 / 100

### Pillar 1: Navigate — 78.25 / 100 (weight: 0.35)

**Poate AI-ul lucra eficient aici?**

| # | Signal | Score | Weight | Evidence |
|---|--------|-------|--------|----------|
| N1 | file_organization | **75** | 0.12 | max_depth=4, directories=2824 |
| N2 | file_granularity | **100** | 0.13 | median_lines=0, large_files=0, per_file_penalty=-0 |
| N3 | module_boundaries | **75** | 0.15 | workspace_files=0, package_manifests=642 |
| N4 | separation_of_concerns | **100** | 0.12 | top_level_dirs=5 (heuristic — override recommended) |
...
| N8 | repo_foundations | **75** | 0.08 | codeowners=1, gitignore_cats=1(10), security_md=1 |
...
| U3 | readme_substance | **80** | 0.15 | readme_lines=493, desc=1, setup=1, usage=1, arch=0, contrib=1 |
...
### Pillar 3: Verify — 15.00 / 100 (weight: 0.30)
...
Readiness_raw = Navigate × 0.35 + Understand × 0.35 + Verify × 0.30
              = 78.25 × 0.35 + 30.25 × 0.35 + 15.00 × 0.30
              = 42.46
...
|-----------|-------|-------|-------------|-------------|------------|
| **Code** | None | 0 | ✓ linter=0, codeowners=1 | ✗ no AI config with Architecture + Conventions | Practice active, no AI config |
| **Testing** | None | 0 | ✓ test execution found in CI workflows | ✗ no AI config with Testing category | Practice active, no AI config |
| **Security** | None | 0 | ✗ dependabot exists but doesn't cover Shell | ✗ no AI config with Security category | No AI presence |
| **Delivery** | None | 0 | ✓ build_workflow=true, issues=true (open=36) | ✗ no AI config with Delivery category | Practice active, no AI config |
...
| Field | Value |
|-------|-------|
| Repository | `IntersectMBO/cardano-haskell-packages` |
| Description | Metadata for Cardano's Haskell package repository |
| Default branch | `main` |
| Private | false |
| Primary language | Shell (62.4%) |
| Languages | Shell: 62.4%,Nix: 36.5%,HTML: 1.1% |
| Size | 545308 KB |
| Open issues | 36 |
| Stars | 36 |
| License | Apache-2.0 |
| Created | 2022-08-22T21:19:53Z |
| Last push | 2026-03-25T09:43:19Z |
| Tree entries | 5660 |
| Source files | 0 |
| Test files | 0 |
| Directories | 2824 |
...
```
Navigate   = 78.25  (weight 0.35)
Understand = 30.25  (weight 0.35)
Verify     = 15.00  (weight 0.30)
...
### Strengthen Verification (Verify: 15.00/100)
