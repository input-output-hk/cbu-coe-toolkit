# AAMM — Sample Report: HarmonicLabs/pluts-emulator
...
| Axis | Score | Detail |
|------|-------|--------|
| **AI Readiness** | **25.83 / 100** | Navigate: 31.29, Understand: 33.25, Verify: 27.50. Penalties: -5. |
...
## AI Readiness: 25.83 / 100

### Pillar 1: Navigate — 31.29 / 100 (weight: 0.35)

**Poate AI-ul lucra eficient aici?**

| # | Signal | Score | Weight | Evidence |
|---|--------|-------|--------|----------|
| N1 | file_organization | **25** | 0.12 | max_depth=2, directories=4 |
| N2 | file_granularity | **98** | 0.13 | median_lines=18, large_files=1, per_file_penalty=-2 |
...
| N6 | cicd_pipeline | **0** | 0.15 | workflows=0, has_build=0, has_deploy=0, days_since_push=76 |
...
### Pillar 2: Understand — 33.25 / 100 (weight: 0.35)
...
| U3 | readme_substance | **100** | 0.15 | readme_lines=250, desc=1, setup=1, usage=1, arch=1, contrib=1 |
...
### Pillar 3: Verify — 27.50 / 100 (weight: 0.30)

**Poate AI-ul verifica ce produce?**

**Hard Gate:** PASS (4 test files)

| # | Signal | Score | Weight | Evidence |
|---|--------|-------|--------|----------|
| V1 | test_source_ratio | **75** | 0.30 | ratio=.571 (4 test / 7 source) |
...
Readiness_raw = Navigate × 0.35 + Understand × 0.35 + Verify × 0.30
              = 31.29 × 0.35 + 33.25 × 0.35 + 27.50 × 0.30
              = 30.83
...
| **Delivery** | None | 0 | ✗ build_workflow=false, issues=true (open=6) | ✗ no AI config with Delivery category | No AI presence |
...
| Field | Value |
|-------|-------|
| Repository | `HarmonicLabs/pluts-emulator` |
| Description | N/A |
| Default branch | `main` |
| Private | false |
| Primary language | TypeScript (90.2%) |
| Languages | TypeScript: 90.2%,JavaScript: 9.8% |
| Size | 354 KB |
| Open issues | 6 |
| Stars | 0 |
| License | Apache-2.0 |
| Created | 2024-07-08T12:29:46Z |
| Last push | 2026-01-08T01:41:19Z |
| Tree entries | 25 |
| Source files | 7 |
| Test files | 4 |
| Directories | 4 |
...
```
Navigate   = 31.29  (weight 0.35)
Understand = 33.25  (weight 0.35)
Verify     = 27.50  (weight 0.30)
...
| V1_V2_mismatch | info | High test ratio (V1=75) but low categorization (V2=25). Many test files exist but few distinct test types detected. May indicate monolithic test suite or detection gaps. | verify_manually |
...
### Improve AI Understanding (Understand: 33.25/100)
...
### Strengthen Verification (Verify: 27.50/100)
